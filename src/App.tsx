...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100 flex overflow-hidden">
      ...
      >
        <div className={`p-6 flex-1 custom-scrollbar ${isCollapsed ? 'overflow-visible' : 'overflow-y-auto'}`}>
        ...
        <div className="mt-auto p-4 border-t border-[#E5E7EB] bg-gray-50/50">
      ...
      >
        ...
        <td className="px-6 py-5 border-r border-slate-50">
          <div className="flex items-center justify-center gap-1.5">
            <div className={`w-2 h-2 rounded-full ${counts.gold > 0 ? 'bg-amber-400 animate-pulse' : 'bg-slate-200'}`} />
...
function MedalBadge({ medal }: { medal: AthleteData['medal'] }) {
  const config = {
    ...
    Silver: { bg: 'bg-slate-50', text: 'text-slate-500', border: 'border-slate-200' },
    Bronze: { bg: 'bg-orange-50', text: 'text-orange-700', border: 'border-orange-200' },
    None: { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200' },
  }[medal];

  return (
    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest border shadow-sm ${config.bg} ${config.text} ${config.border}`}>
